...
type kubernetesReplicaSetReader interface {
	...
	ListReplicaSetsForCluster(ctx context.Context, clusterID uuid.UUID, organizationID uuid.UUID, namespace string) (*dto.ReplicaSetListResponse, error)
}
...
func (h *KubernetesReplicaSetHandler) ListByCluster(c *gin.Context) {
	...
	namespace := strings.TrimSpace(c.Query("namespace"))
	result, err := h.service.ListReplicaSetsForCluster(c.Request.Context(), clusterID, organizationID, namespace)
	if err != nil {
