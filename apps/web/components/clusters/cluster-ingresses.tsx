...
import { useIngressesByCluster } from "@/hooks/use-ingresses";

export function ClusterIngresses({ clusterId }: { clusterId: string }) {
  const params = useMemo(() => ({}), []);
  const { data, error, isError, isLoading, refetch } = useIngressesByCluster(clusterId, params);
  ...
  if (ingresses.length === 0) {
    return (
      <EmptyState
        ...
        description="Ingresses discovered from this cluster will appear here."
      />
    );
  }
