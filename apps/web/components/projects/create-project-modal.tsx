...
export function CreateProjectModal({ open, onClose, onCreated }: CreateProjectModalProps) {
  ...
  return (
    <dialog
      ...
      aria-describedby={submitError ? "create-project-description create-project-submit-error" : "create-project-description"}
      ...
      <form onSubmit={handleSubmit(submit)}>
        <header className="flex items-start justify-between border-b p-5" style={{ borderColor: "var(--border)" }}>
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p id="create-project-description" className="mt-1 text-sm" style={{ color: "var(--muted-foreground)" }}>
        ...
        <div className="space-y-5 p-5">
          <div>
            ...
            />
            {errors.name && (
              <p id="project-name-error" className="mt-1.5 text-xs" style={{ color: "var(--danger)" }}>
                {errors.name.message}
              </p>
            )}
          </div>
          ...
          <div>
            ...
            />
            {errors.description && (
              <p id="project-description-error" className="mt-1.5 text-xs" style={{ color: "var(--danger)" }}>
          ...
          {submitError && (
            <p
              id="create-project-submit-error"
              role="alert"
              aria-live="assertive"
              className="text-sm"
        ...
        <footer className="flex justify-end gap-3 border-t p-5" style={{ borderColor: "var(--border)" }}>
          <Button type="button" variant="secondary" onClick={close} disabled={createProject.isPending}>
            Cancel
          </Button>
...
function getProjectErrorMessage(error: unknown) {
  if (axios.isAxiosError<{ message?: string }>(error)) {
    return error.response?.data?.message ?? "Unable to create project. Please try again.";
  }
