...
import { useAssignProjectTeam, useProjectTeams, useRemoveProjectTeam } from "@/hooks/use-project-teams";
...
/** Project ↔ Team assignment. Mounted only when the Members tab is active. */
export function ProjectTeams({ projectId }: { projectId: string }) {
  ...
  const { data, error, isError, isLoading, refetch } = useProjectTeams(projectId);
  ...
  const handleAssign = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      await assignTeam.mutateAsync({ projectId, payload: { teamId: teamToAssign } });
      setTeamToAssign("");
  ...
  if (isError) {
    return (
      <ErrorState
        description={error instanceof Error ? error.message : "Unable to load project teams. Please try again."}
        onRetry={() => {
          void refetch();
        }}
      />
    ...
    <div className="space-y-5">
      {isAdmin ? (
        <form onSubmit={handleAssign} className="flex items-end gap-2">
          <div className="flex-1">
            <label htmlFor="assign-team-select" className="text-xs" style={{ color: "var(--muted-foreground)" }}>
              Assign a team
            </label>
            <select
      ...
      {assignments.length === 0 ? (
        <EmptyState
          ...
          description="Assign a team above to grant it access to this project."
        />
      ) : (
                ...
                <div className="flex items-start justify-between gap-3">
                  ...
                  {isAdmin ? (
                    <Button
                      ...
                      aria-label={`Remove ${team?.name ?? assignment.teamId} from project`}
                    >
